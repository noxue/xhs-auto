...
// like-notes
document.getElementById("like-notes").addEventListener('click', () => {
	...
	setInterval(() => {
		...
		db.queryAllNotes(page, 1, (err, rows) => {
			...
			notes.forEach(note => {
				// 点赞
				ipcRenderer.send('like', note.noteId);
			})
...
ipcRenderer.on('comment-data', (event, data) => {
	...
	comments.forEach(comment => {
		...
		let isAuthor = 0;
		if(comment.show_tags.length > 0 && comment.show_tags[0] == "is_author"){
			isAuthor = 1;
		}
		...
		if(comment.sub_comments.length > 0){
			comment.sub_comments.forEach(subComment => {
				let isAuthor = 0;
				if(subComment.show_tags.length > 0 && subComment.show_tags[0] == "is_author"){
					isAuthor = 1;
				}
                db.insertCommentData(new Comment(
                    subComment.note_id,
